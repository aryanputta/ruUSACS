...
Tests cover:
  ...
  3. GET  /api/commute/health     -- health-check.
  4. GET  /api/commute/config     -- public config + key-status flags.
  5. POST /api/commute/optimize   -- validation errors (missing fields).
  6. POST /api/commute/optimize   -- LIVE call to Azure Maps Route Matrix API
                                     (Rutgers campus: drive + walk).

Run with:
    cd /workspace && python3 -m pytest backend/tests/test_api.py -v
"""
...
class TestKeyLoading:
    ...
    def test_azure_maps_subscription_key_loaded(self):
        ...
        assert len(settings.azure_maps_subscription_key) > 10
...
class TestRoot:
    ...
    async def test_root_returns_service_info(self, client: httpx.AsyncClient):
        ...
        assert "POST /api/commute/optimize" in data["endpoints"]
        assert "GET  /api/commute/health" in data["endpoints"]
        assert "GET  /api/commute/config" in data["endpoints"]


# ---------------------------------------------------------------------------
# 3. GET /api/commute/health
# ---------------------------------------------------------------------------
...
class TestHealth:
    ...
    async def test_health_returns_200(self, client: httpx.AsyncClient):
        resp = await client.get("/api/commute/health")
        assert resp.status_code == 200
        data = resp.json()
...
class TestConfig:
    ...
    async def test_config_returns_client_id(self, client: httpx.AsyncClient):
        resp = await client.get("/api/commute/config")
        assert resp.status_code == 200
        data = resp.json()
        assert data["success"] is True
...
# ---------------------------------------------------------------------------
# 5. POST /api/commute/optimize  -- validation errors
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. POST /api/commute/optimize  -- LIVE Azure Maps call
# ---------------------------------------------------------------------------
...
class TestOptimizeLive:
    ...
    async def test_optimize_returns_valid_result(
        ...
        resp = await client.post("/api/commute/optimize", json=self.PAYLOAD)
        assert resp.status_code == 200, (
